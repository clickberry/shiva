...
Bus.prototype.listen = function () {
  ...
  new nsq.Reader('shiva-processed-video-segments', 'concat-video', {
    lookupdHTTPAddresses: lookupdHTTPAddresses
    })
    .on('message', function (msg) {
      setTimeout(touch, msg.timeUntilTimeout() - 1000, msg, 'shiva-processed-video-segments');
      bus.emit('processed-video-segments', msg);
    })
    .connect();    
};
...
Bus.prototype.publishAllSegmentsProcessed = function (data, fn) {
  this._writer.publish('shiva-processed-video-segments', data, fn);
};
